...
subset_salarios %>%
  mutate(tempo_ingresso = year(Sys.Date()) - year(DATA_DIPLOMA_INGRESSO_SERVICOPUBLICO)) %>%
  summarise(tempo_medio = mean(tempo_ingresso)) %>%
  select(tempo_medio)
...
subset_salarios %>%
  group_by(UF_EXERCICIO) %>%
  summarise(
    media_salario = mean(REMUNERACAO_REAIS),
    mediana_salario = median(REMUNERACAO_REAIS),
    media_maior = media_salario > mediana_salario
  ) %>%
  ungroup()  %>%
  count(media_maior) -> subset_media_mediana
...
#' para distribuições de probabilidade onde o Desvio Padrão é definido, 2 Desvios Padrão da média devem absorver pelo
